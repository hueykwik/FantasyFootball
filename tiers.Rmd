...
TODO: How do we test accuracy better?
Bootstrap, leave out 20%, see if it still does better
...
Perhaps I can train = 2015 season, test = 2014 season or vice versa
...
# Example for week 17 DST
test_df = ecr_scores %>% select(playername, points, actualPoints) %>% arrange(desc(points))
n_tiers = 7
best_indexes = bestPartition(n_tiers, test_df$actualPoints, test_df$points)
expand(1:n_tiers, best_indexes, nrow(test_df))
```

```{r random partition tiers}
for (week in seq(17)) {
  ...
  for (pos in toupper(pos.list)) {
    ...
    best_indexes = bestPartition(kTiers[pos], test_df$actualPoints, test_df$points)
    tiers = expand(1:kTiers[pos], best_indexes, nrow(test_df))
    print(tiers)
