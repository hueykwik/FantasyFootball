...
## Computing Tiers based on ECR

I wrote some code to replicate Chen's work for Week 17. 

Notes:
* Not all players are looked at, just the top listed in `cutoffs` below.
* Number of tiers are not calculated automatically. 

```{r}

cutoffs = c(QB = 24, RB = 40, WR = 60, TE = 24, K = 24, DST = 24)
tiers = c(QB = 8, RB = 9, WR = 12, TE = 8, K = 5, DST = 6)

# Load Week 17 since that's published
ecr_df = read_tsv("data/2015/week_17_qb.tsv")
ecr_df = ecr_df[1:cutoffs["QB"], ]
names(ecr_df) = make.names(names(ecr_df))

# Calculate tiers based on his code
kNumClusters = 8
fit = Mclust(ecr_df$Avg.Rank, G = kNumClusters)
...
# If this works reasonably well, then record my tiers in a file
...
for (week in seq(17)) {
  ...
  for (pos in toupper(pos.list)) {
    ...
    # TODO(huey): Only do tiers for the top players -- need to get the top players by fantasy points
    ...
    sdf[sdf$position == pos, ]$TierProj[1:kCutoffs[pos]] = pos_df$TierProj  
    
    ## Calculate TierECR
      
    # Load ECR data for the week
    ecr_df = read_tsv(fn$identity("data/2015/week_`week`_`tolower(pos)`.tsv"))
    ecr_df = ecr_df[1:cutoffs[pos], ]  # TODO(huey): Need to only get the top players, need to do an order by
    names(ecr_df) = make.names(names(ecr_df))
    ...
    sdf[sdf$position == pos, ]$TierECR[1:kCutoffs[pos]] = ecr_df$TierECR    
    ecr_df$nchar = nchar(as.character(ecr_df$Player.Name))  # For formatting later
  ...
  full_df = rbind(full_df, sdf)
}

full_df = full_df %>% select(-player, -cost, -salary, -adp, -adpdiff, -auctionValue, -overallECR, -sleeper)
```
...
Super ugh! Boris's published stuff only shows Week 17, so I can check against
that for now. Then again, I have his source code, so I could run against that.
