...
bootstrapCompare = function(n) {
  ...
  for (week in seq(17)) {
    ...
    for (pos in toupper(pos.list)) {
      ...
      for (i in seq(n)) {
        ...
        # select(pos_df, playername, points, actualPoints, TierProj)  # For debugging
        pos_df$TierProj = computeTiers(pos_df$points, kTiers[pos], reverse = TRUE)
    
        ## Calculate TierECR
        ecr_df = sample %>% arrange(Rank)
        ecr_df = ecr_df[1:kCutoffs[pos], ]
        #select(ecr_df, playername, points, actualPoints, Avg.Rank)  # For debugging
        ecr_df$TierECR = computeTiers(ecr_df$Avg.Rank, kTiers[pos])
  ...
  sapply(results_df, mean)
}
```

```{r bootstrap results}
rm(week)
set.seed(1); bootstrapCompare(1)  # works
set.seed(1); bootstrapCompare(10)  # works
set.seed(1); bootstrapCompare(50)
tiersset.seed(1); bootstrapCompare(100)
set.seed(1); bootstrapCompare(200)
```
