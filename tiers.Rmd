...
read_ecr_data = function(path, n_players) {
  ecr_df = read_tsv(path) %>% arrange()
  ecr_df = ecr_df[1:n_players, ]  # The data is ordered from best rank to worst rank  
...
computeTierECRs = function(ranks, k) {
  ...
  # Remove any potential gap between tiers, e.g. 1 1 2 2 4
...
for (week in seq(17)) {
  ...
  for (pos in toupper(pos.list)) {
    ...
    ecr_df = read_ecr_data(fn$identity("data/2015/week_`week`_`tolower(pos)`.tsv"), kCutoffs[pos])
