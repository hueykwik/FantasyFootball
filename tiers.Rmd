...
* TierRank doesn't match Chen's completely. 
...
createProjectionPlot = function(pos_df, players_to_label) {
...
createWeek17QBPlot = function(pos_df) {
  ...
  createProjectionPlot(pos_df, players_to_label)
}
...
for (week in seq(17)) {
  ...
  for (pos in toupper(pos.list)) {
    ...
    if (plot) {
      p = ggplot(pos_df, aes(x = positionRank, y = points))
      p = p + geom_text(aes(label=playername, colour=TierProj), size=font,  position = position_jitter(width = 0.25, height = 0.25))
      p = p + scale_x_continuous("Position Rank")
      p = p + ylab("Projected Points")
      p
      
      createWeek17QBPlot(pos_df)
...
scores, and "TierRank" will refer to creating tiers based on Expert Consensus Rank. 
...
n1# set.seed(1); n200 = bootstrapCompare(200) # w
# set.seed(1); n500 = bootstrapCompare(500) # w
set.seed(1); n1000 = bootstrapCompare(1000)
tic()
set.seed(1); n10000 = bootstrapCompare(10000)
toc()
...
What % of time does TierProjections perform better than TierRank?
