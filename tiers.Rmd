...
for (week in seq(17)) {
  ...
  for (pos in toupper(pos.list)) {
    ...
    week_tier_proj_accuracy  = c(week_tier_proj_accuracy, computeAccuracy(pos_df$actualPoints, as.numeric(pos_df$TierProj)))
    week_tier_ecr_accuracy = c(week_tier_ecr_accuracy, computeAccuracy(ecr_scores$actualPoints, as.numeric(ecr_scores$TierECR)))
  }
  ...
  select(pos_df, playername, TierProj, actualPoints)
  select(ecr_scores, Player.Name, TierECR, actualPoints)
  
  names(tier_proj_accuracy) = pos.list
  names(tier_ecr_accuracy) = pos.list
  
  mean_proj = sapply(tier_proj_accuracy, mean)
  mean_tier = sapply(tier_ecr_accuracy, mean)
}

computeAccuracy(as.numeric(pos_df$actualPoints), as.numeric(pos_df$TierProj))
```
...
names(week_13_scores)[9] = "Rush"
names(week_13_scores)[10] = "RushYard"
names(week_13_scores)[11] = "RushTD"

ecr_scores = inner_join(week_13_ecr, week_13_scores, by = c("Player" = "player"))
tier_ecr = Mclust(ecr_scores$Ave, G = kTiers["QB"])
ecr_scores$TierECR = tier_ecr$class
...
computeAccuracy(ecr_scores$FPts, ecr_scores$TierECR)

week_10 = read_delim("data/2013/week-10-qb-tiers-points", delim = " ")
computeAccuracy(week_10$points, week_10$tier)

tiers = c(1,2,3,4,5)
medians = c(27.6, 23.9, 14, 16.7, 8.9)

computeAccuracy(medians, tiers)
```
